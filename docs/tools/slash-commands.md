...
- `/watch models <model> until available`, `/watch url <url> contains "<text>"`, `/watch url <url> matches "<regex>"`, `/watch url <url> changed`, `/watch github pr <url|owner/repo#number> until checks pass`, and `/watch github pr <url|owner/repo#number> changed` create temporary watches. GitHub PR `changed` means the PR snapshot changed, including state, draft/merged state, head SHA, or checks rollup. `/watches` lists active watches, `/watch show <id>` shows details, and `/watch cancel <id>` cancels one. See [Watches](/tools/watches).
