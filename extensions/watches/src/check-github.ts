...
function snapshotHash(params: { pr: GitHubPrApiResponse; checks: GitHubChecksRollup }): string {
...
function formatSnapshotSummary(params: {
  ...
  return truncateSummary(
    `PR snapshot: ${formatGitHubPrRef(params.source)} — ${params.pr.title} | State: ${prStateLabel(
      params.pr,
    )} | Checks: ${checksLabel(params.checks)} | Head: ${shortSha(params.pr.headSha)}`,
  );
}
...
export function formatGitHubPrNotification(params: {
  kind: "checks_passed" | "snapshot_changed";
  ...
  const heading =
    params.kind === "checks_passed" ? "✅ PR checks passed" : "👀 PR snapshot changed";
  const lines = [
    heading,
    "",
...
export async function fetchGitHubPrSnapshot(params: {
  ...
  const summary = formatSnapshotSummary({ source: params.source, pr, checks });
  return {
    ...
    checks,
    summary,
    resultHash: snapshotHash({ pr, checks }),
  };
}
